...
class CoHereProvider(LLMInterface):
    ...
    def __init__(self,api_key:str ,
        ...
        # AsyncClient (not the sync client) so generation/embedding calls
        # don't block the event loop. 240s bound guards against a stalled
        # upstream (relevant when embeddings are proxied through ngrok).
        self.client = cohere.AsyncClient(self.api_key, timeout=240) if self.api_key else None
    ...
    async def generate_text(self, prompt: str,
        ...
        try:
            ...
            self.logger.error(f"Cohere generate_text failed: {e}")
            return None
    ...
    async def embed_text(self, text: Union[str , List[str]], document_type: str = None):
        ...
        # `document_type` is always the *value* of DocumentType (e.g. "query"),
        # not the Enum member itself — compare against `.value`, otherwise this
        # never matches and every embedding (queries included) is sent as
        # input_type="search_document", which silently degrades retrieval
        # quality for Cohere's asymmetric embedding models.
        ...
        try:
            ...
            self.logger.error(f"Cohere embed_text failed: {e}")
            return None
